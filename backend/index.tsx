...
// Supabase client (currently unused, but available for future features)
...
// Defensive JSON parsing helper (prevents "unexpected character" errors)
async function readJsonOrThrow(res: Response) {
  const contentType = res.headers.get("content-type") || "";
  const raw = await res.text(); // ALWAYS read as text first
  const trimmed = raw.replace(/^\uFEFF/, "").trim(); // strip BOM

  // If not OK, throw with payload preview (this alone saves hours)
  if (!res.ok) {
    throw new Error(
      `HTTP ${res.status} ${res.statusText} | content-type=${contentType} | body=${trimmed.slice(0, 300)}`
    );
  }

  // Handle empty body (204 or sometimes 200 with no content)
  if (!trimmed) return null;

  // Common non-JSON prefixes some systems/proxies add
  const cleaned = trimmed
    .replace(/^\)\]\}',?\s*\n/, "")          // anti-XSSI prefix
    .replace(/^null(?=\s*[{[])/, "");       // strips leading "null" ONLY if JSON follows

  try {
    return JSON.parse(cleaned);
  } catch (e: any) {
    throw new Error(
      `JSON parse failed | content-type=${contentType} | first200=${cleaned.slice(0, 200)} | err=${e?.message}`
    );
  }
}

// Vintrace request helper (similar to commerce7Request)
async function vintraceRequest(path: string, opts: RequestInit = {}) {
  if (!VINTRACE_API_KEY) {
    throw new Error("Vintrace API key not configured");
  }

  const url = `${VINTRACE_BASE_URL}${path}`;
  console.log(`[Vintrace] ${opts.method || 'GET'} ${url}`);
...
app.get("/make-server-920a8867/commerce7/test-auth", async (c) => {
  const testResults = [];
...
app.get("/make-server-920a8867/commerce7/orders", async (c) => {
  try {
...
// Get sales metrics
app.get("/make-server-920a8867/commerce7/sales-metrics", async (c) => {
  try {
...
// Get customer data
app.get("/make-server-920a8867/commerce7/customers", async (c) => {
  try {
...
app.get("/make-server-920a8867/commerce7/curated-products", async (c) => {
  try {
...
app.get("/make-server-920a8867/dash/exec/overview", async (c) => {
  try {
...
app.get("/make-server-920a8867/dash/exec/top-movers", async (c) => {
  try {
...
const DASH_LOCK_KEY = 'dash_exec_lock_v1';
...
async function tryAcquireLock(): Promise<boolean> {
  try {
    const existingLock = await kv.get(DASH_LOCK_KEY) as { timestamp: number } | null;
    
    if (existingLock) {
      const lockAge = Date.now() - existingLock.timestamp;
      
      // If lock is stale (older than TTL), we can steal it
      if (lockAge > DASH_LOCK_TTL_MS) {
        console.log(`[Lock] Stale lock detected (age: ${Math.floor(lockAge / 1000)}s), stealing it`);
      } else {
        console.log(`[Lock] Lock already held (age: ${Math.floor(lockAge / 1000)}s)`);
        return false;
      }
    }
    
    // Acquire lock
    await kv.set(DASH_LOCK_KEY, { timestamp: Date.now() });
...
console.log("[DEBUG] curatedSkuCount:", curatedSkus.size);
  ...
  console.log(`[Compute] Total orders fetched: ${orders.length}`);
  console.log(`[Compute] Orders in last 30 days: ${orders30d.length}`);
  console.log(`[Compute] Orders in last 90 days: ${orders90d.length}`);
  
  // Log date range for debugging
  if (orders.length > 0) {
    const dates = orders.map(o => getOrderDate(o)).filter(Boolean);
    const oldest = new Date(Math.min(...dates.map(d => d!.getTime())));
    const newest = new Date(Math.max(...dates.map(d => d!.getTime())));
    console.log(`[Compute] Order date range: ${oldest.toISOString()} to ${newest.toISOString()}`);
  }
  
  // Log February 2026 orders specifically
  const feb2026Orders = orders.filter(o => {
    const d = getOrderDate(o);
    return d && d.getFullYear() === 2026 && d.getMonth() === 1; // Month 1 = February
  });
  console.log(`[Compute] February 2026 orders found: ${feb2026Orders.length}`);
  if (feb2026Orders.length > 0) {
    console.log(`[Compute] Sample February order:`, JSON.stringify(feb2026Orders[0]).substring(0, 300));
  }
  ...
  for (const order of orders90d) {
    ...
    // Debug February 2026 allocations
    if (d.getFullYear() === 2026 && d.getMonth() === 1) {
      console.log(`[Compute] Feb 2026 order allocation:`, {
        orderId: order.id,
        orderTotal: order.total,
        allocRevenue: alloc.revenue,
        allocBottles: alloc.bottles,
        skuCount: Object.keys(alloc.skuRevenue).length
      });
    }
...
app.get("/make-server-920a8867/dash/exec/audit", async (c) => {
...
app.get("/make-server-920a8867/dash/exec/test-apis", async (c) => {
  const results: any = {
...
// Vintrace inventory endpoint
app.get("/make-server-920a8867/vintrace/inventory", async (c) => {
  try {
...
app.post("/make-server-920a8867/dash/exec/clear-cache", async (c) => {
...
app.get("/make-server-920a8867/dash/exec", async (c) => {
...
app.get("/make-server-920a8867/vintrace/search", async (c) => {
  try {
...
app.get("/make-server-920a8867/vintrace/batches", async (c) => {
  try {
...
app.get("/make-server-920a8867/vintrace/wine-batches", async (c) => {
  try {
    const bbvOnly = c.req.query("bbvOnly") === "true";
...
app.get("/make-server-920a8867/vintrace/vessel-details", async (c) => {
  try {
...
app.get("/make-server-920a8867/vintrace/debug", async (c) => {
  try {
...
app.get("/make-server-920a8867/vintrace/stock-dispatches", async (c) => {
  try {
    const startDate = c.req.query("startDate");
    const endDate = c.req.query("endDate");
